...
QStringList getSysIndexes(const QString & table, const QString & schema)
{
        QStringList orig = getObjects("index", schema).values(table);
        // really all indexes
        QStringList sysIx;
...
DbAttach getDatabases()
{
        DbAttach ret;
        QSqlQuery query("PRAGMA database_list;");
...
{
        QStringList databases(getDatabases().keys());
	clear();
...
void TableTree::buildTables(QTreeWidgetItem * tablesItem, const QString & schema)
{
	deleteChildren(tablesItem);
        ...
        QStringList tables = getObjects("table", schema).keys();
...
void TableTree::buildIndexes(QTreeWidgetItem *indexesItem, const QString & schema, const QString & table)
{
	deleteChildren(indexesItem);
        QStringList values = getObjects("index", schema).values(table);
...
void TableTree::buildColumns(QTreeWidgetItem * columnsItem, const QString & schema, const QString & table)
{
	deleteChildren(columnsItem);
        FieldList values = tableFields(table);
...
{
	deleteChildren(indexesItem);
        QStringList sysIx = getSysIndexes(table, schema);
...
void TableTree::buildTriggers(QTreeWidgetItem *triggersItem, const QString & schema, const QString & table)
{
	deleteChildren(triggersItem);
        QStringList values = getObjects("trigger", schema).values(table.toLower());
...
void TableTree::buildViews(QTreeWidgetItem * viewsItem, const QString & schema)
{
	...
	// Build views tree
        QStringList views = getObjects("view", schema).keys();
